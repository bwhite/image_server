...
def find_local_images():
    ...
    images_grouped = {}
    for image in images:
        re_out = GROUP_RE.search(image)
        re_groups = re_out.groups() if re_out else []
        group_key = ', '.join(re_groups)
...
def main(auth_key, page=''):
    ...
    templ = os.path.join(os.path.dirname(__file__), 'image_serve_template.tpl')
